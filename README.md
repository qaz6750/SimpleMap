...
- 最近搜索保存在本机（最多 10 条），可单条删除或一键清空，再次点击即可重新定位。
- 支持实时路况、卫星图、定位、组合缩放控件和随镜头变化的 Compose 比例尺。
- 可在 2D、3D 视角间切换，并一键恢复地图正北；图层选择后自动收起操作面板。
- 长按地图任意位置即可选点，直接查看坐标详情并发起路线规划。
- 支持家、公司和自定义收藏分组，可从搜索结果或收藏直接发起路线规划。
- 设置家或公司后，首页搜索栏下方会出现“回家”“去公司”通勤捷径，一键进入驾车规划。
- 地点详情支持周边快捷搜索（停车场、加油站、充电站、美食）与系统分享。
...
- 导航通知实时显示转向指令、剩余里程/时间和预计到达；Android 16+ 升级为 Live Updates 实时进度通知（状态栏芯片与路线进度条）。
- 授予悬浮窗权限后，退出应用会显示可拖动的悬浮导航卡片，展示下一步转向与剩余信息，点击即可返回导航。
- 导航设置提供"后台导航保护"入口，引导加入电池优化白名单；划掉最近任务卡片不会中断进行中的导航。
- 语音支持详细、简洁和静音三级，并提供跨午夜静音时段与独立的重要提示开关。

### 行程与本地能力

- “我的”采用纵向入口列表，收藏地点在分组内以紧凑行展示，适合快速扫描和重复操作。
- 行程历史记录到达、取消和失败状态，以及真实耗时、里程和平均速度；模拟导航会被明确标记，支持左滑或在详情中删除单条行程。
...
- Release 构建启用 R8 代码压缩，当前未启用资源裁剪，且默认产物未签名；正式分发前需配置独立签名。
