...
- 统一蓝色的竖屏手机和横屏车机自适应导航：高德 SDK 官方路口放大图出现后直接展示，醒目的辅助行驶指令、左侧高速出口、限速、电子眼距离、区间测速剩余距离与建议速度；实时路况持续更新前方缓行和拥堵距离，并显示当前路线最近的事故、施工或道路封闭详情。限行、避堵、路线更新和途经点到达使用自动消退的路线事件提示。点击沿途卡片可在方向自适应悬浮窗中滚动查看全路线服务区和收费站。GPS 详情使用同一悬浮窗规则并在 5 秒后自动关闭，导航设置支持持久化日间/夜间模式。
